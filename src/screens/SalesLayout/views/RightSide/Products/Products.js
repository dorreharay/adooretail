...
function Products(props) {
  ...
  const [savedActiveCategoryPath, setSavedActiveCategoryPath] = useState({
    index: 0,
    key: 0,
  });
  ...
  useEffect(() => {
    if (activeCategory && account?.products) {
  ...
  const resetCategory = () => {
    ...
    const flattened = account?.products.flat();
  ...
  useEffect(() => {
    if (searchTerm.length > 0) {
      ...
      let newProducts = account?.products
        .flat()
        .map(item => ({
          ...item,
            ...
            <View style={[styles.row]} key={index}>
              {row &&
                row.map((rowItem, key) => (
                  <TouchableOpacity
                    style={[
                      styles[`colsProduct${layout}`],
                      { height: calculateColHeight(layout) },
                      key === 0 && { marginLeft: 0 },
                    ]}
                  ...
                  >
                    {rowItem.img_url !== '' ? (
                      ...
                      <LinearGradient
                        ...
                        colors={[
                          rowItem ? rowItem.placeholder_color : '#CCC',
                          rowItem ? rowItem.placeholder_shadow : '#CCC',
                        ]}
                        start={{ x: 0, y: 0 }}
                        end={{ x: 1, y: 0 }}
                      />
                    ...
                    >
                      <Text
                        numberOfLines={2}
                        ellipsizeMode="tail"
                        style={styles[`categoryTitleText${layout}`]}
                      >
                        {rowItem.title}
                      </Text>
                    </View>
                            ...
                            >
                              ...
                              rowItem.size ? (
                                <View style={styles[`variantPrice${layout}`]}>
                                  <Text
                                    style={styles[`variantPriceText${layout}`]}
                                  >
                                    {rowItem.size}
                                  </Text>
                                </View>
                              ...
                              rowItem.size ? (
                                <View
                                  style={[
                                    styles[`variantSize${layout}`],
                                    { backgroundColor: rowItem.color },
                                  ]}
                                >
                                  <Text
                                    style={[
                                      styles[`categoryTitleText${layout}`],
                                      { color: '#FFFFFF' },
                                    ]}
                                  >
                                    {rowItem.size}
                        ...
                        >
                          ...
                          <View
                            style={{
                              ...
                              width: layout === 3 ? 65 : 35,
                            }}
                          >
                            <Text style={styles[`categoryTitleText${layout}`]}>
                              {rowItem.price}₴
                            </Text>
                          </View>
                        </TouchableOpacity>
