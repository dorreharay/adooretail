...
function History(props) {
  ...
  useEffect(() => {
    Animated.timing(animatedSideValue, {
  ...
  return (
    <View style={styles.container}>
      <Filters toastRef={toastRef} setSideMenuStatus={setSideMenuStatus} />
      ...
      <Animated.View
        style={[styles.sideMenuContainer, { right: animatedSideValue }]}
      >
        <TouchableOpacity
          style={styles.sideMenuWrapper}
        ...
        <View style={styles.sideMenu}>
          ...
          {dates.map((item, index) => (
            <TouchableOpacity
              style={[
                styles.button,
                historyParams.date === getSubstractDate('YYYY-MM-DD', index) &&
                  styles.activeButton,
              ]}
              onPress={() => {
                ...
                dispatch(
                  setHistoryParams({
                    ...historyParams,
                    date: getSubstractDate('YYYY-MM-DD', index),
                  }),
                );
              }}
              activeOpacity={0.8}
            ...
            >
              <Text
                style={[
                  styles.buttonText,
                  historyParams.date ===
                    getSubstractDate('YYYY-MM-DD', index) &&
                    styles.activeButtonText,
                ]}
              >
                {loadingIndex === index ? 'Оновлення...' : item}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <HistoryList loading={loading} setLoadingStatus={setLoadingStatus} />
    </View>
  );
}
